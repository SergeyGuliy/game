...
const Social = ({ color, fill = "#F8BB46", background = "#fff" }) => {
  const [atPath, setPath] = React.useState("");
  React.useEffect(() => {
    setPath(window.location.origin);
  }, []);

  const vk = `https://vk.com/share.php?url=${atPath}&title=The%20Body%20Shop&comment=ЗАМУР-Р-РЧАТЕЛЬНЫЕ БАТТЕРЫ! Помоги котику собрать ингредиенты из новой коллекции масел для тела «The Body Shop», получи промо-код на скидку и получи возможность участвовать в розыгрыше наборов из масел для тела&description=ЗАМУР-Р-РЧАТЕЛЬНЫЕ БАТТЕРЫ! Помоги котику собрать ингредиенты из новой коллекции масел для тела «The Body Shop», получи промо-код на скидку и получи возможность участвовать в розыгрыше наборов из масел для тела&image=https://iskra-st.ru/img/IMG_2734.PNG`;
  const fb = `https://www.facebook.com/sharer/sharer.php?u=${atPath}&title=The%20Body%20Shop&=description=ЗАМУР-Р-РЧАТЕЛЬНЫЕ%20БАТТЕРЫ!%20Помоги%20котику%20собрать%20ингредиенты%20из%20новой%20коллекции%20масел%20для%20тела%20«The%20Body%20Shop»,%20получи%20промо-код%20на%20скидку%20и%20получи%20возможность%20участвовать%20в%20розыгрыше%20наборов%20из%20масел%20для%20тела&picture=https%3A%2F%2Fiskra-st.ru%2Fimg%2FIMG_2734.PNG`;
  const tw = `https://twitter.com/intent/tweet?url=${atPath}&text=The%20Body%20Shop`;
  const ok = `https://connect.ok.ru/dk?st.cmd=WidgetSharePreview&st.shareUrl=${atPath}`;
  return (
    <>
